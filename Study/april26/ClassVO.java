...
public class ClassVO {
	...
	public void addStudent(String stuNum, String name, int kor, int eng, int mat, int edps) {
		if(index<studentVOs.length) studentVOs[index++] = new StudentVO(stuNum, name, kor, eng, mat, edps);
	}
	// 학번으로 조회하기
	public StudentVO getStudentVO(String stuNum) {
		StudentVO vo = null;
		ranking(); // 석차를 새로 구하고
		for(StudentVO v : studentVOs) {
			if(v!=null && v.getStuNum().equals(stuNum)) {
				vo = v;
				break;
			}
		}
	...
	private void ranking() {
		...
		for(int i=0; i<index-1;i++) {
			for(int j=i+1; j<index; j++) {
				...
				}else if(studentVOs[i].getTotal()<studentVOs[j].getTotal()) { // j의 점수가 크면 
