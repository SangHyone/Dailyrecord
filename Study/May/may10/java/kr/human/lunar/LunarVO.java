...
public class LunarVO {
	...
	// 윤달이 없는 경우
	// 신축(辛丑)년 신축(辛丑)월 기미(己未)일)
	// 세차 월건 일진

	// 년도
	public String getGanjiYearKo() { // 한글로 얻기
		return ganji.split(" ")[0].substring(0, 2);
	}

	public String getGanjiYearCn() { // 한자로 얻기
		return ganji.split(" ")[0].substring(3, 5);
	}

	// 월
	public String getGanjiMonthKo() { // 한글로 얻기
	...
	public String getGanjiMonthCn() { // 한자로 얻기
		if (ganji.split(" ").length == 3) {
	...
	// 일진
		public String getGanjiDayKo() { // 한글로 얻기
			if (ganji.split(" ").length == 3) {
		...
		public String getGanjiDayCn() { // 한자로 얻기
			if (ganji.split(" ").length == 3) {
